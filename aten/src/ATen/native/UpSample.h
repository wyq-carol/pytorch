...
template <typename scalar_in, typename scalar_out,
          typename std::enable_if_t<!vec::is_reduced_floating_point_v<scalar_out> || !std::is_same<scalar_in, float>::value, int> = 0>
void inline apply_grad_input(scalar_in* buffer_ptr, scalar_out* gin, int64_t size) {
  TORCH_CHECK((vec::is_reduced_floating_point_v<scalar_out>),
...
template <typename scalar_in, typename scalar_out,
          typename std::enable_if_t<vec::is_reduced_floating_point_v<scalar_out> && std::is_same<scalar_in, float>::value, int> = 0>
void inline apply_grad_input(scalar_in* buffer_ptr, scalar_out* gin, int64_t size) {
  using bVec = vec::Vectorized<scalar_out>;
  using fVec = vec::Vectorized<float>;
  int64_t d = 0;
  for (; d < size - (size % bVec::size()); d += bVec::size()) {
    bVec gin_bvec = bVec::loadu(gin + d);
    fVec gin_fvec0, gin_fvec1;
    std::tie(gin_fvec0, gin_fvec1) = vec::convert_to_float<scalar_out>(gin_bvec);
    ...
    vec::convert_from_float<scalar_out>(gin_fvec0, gin_fvec1).store(gin + d);
  }
  for (; d < size; d++) {
    gin[d] += buffer_ptr[d];
    buffer_ptr[d] = 0;
