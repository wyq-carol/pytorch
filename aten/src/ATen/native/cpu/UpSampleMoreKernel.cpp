...
template <typename acc_t, typename scalar_t,
          typename std::enable_if_t<!vec::is_reduced_floating_point_v<scalar_t> || !std::is_same<acc_t, float>::value, int> = 0>
...
void inline nearest_channels_last_acc(acc_t* gin, scalar_t* gout, int64_t size) {
  ...
  for (; d < size - (size % bVec::size()); d += bVec::size()) {
    ...
    auto [gout_fvec0, gout_fvec1] = vec::convert_to_float<scalar_t>(gout_bvec);
