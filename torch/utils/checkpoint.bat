...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        ...
        # gpu_tracker = MemTracker(frame, path='/home/code/tmp/')
    ...
    def backward(ctx, *args):
        ...
        with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
            ...
            detached_inputs = detach_variable(tuple(inputs)) # Tensor+ allocate_mem=
            with torch.enable_grad(), \
                 torch.cuda.amp.autocast(**ctx.gpu_autocast_kwargs), \
                 torch.cpu.amp.autocast(**ctx.cpu_autocast_kwargs):
                outputs = ctx.run_function(*detached_inputs) # Tensor+ allocate_mem=
                # print("------backward---------")
        ...
        torch.autograd.backward(outputs_with_grad, args_with_grad) # Tensor= allocate_mem-
